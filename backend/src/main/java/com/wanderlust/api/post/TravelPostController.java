...
public class TravelPostController {
    ...
    public ResponseEntity<PagedResponse<PostDto>> getNearby(
            ...
            @RequestParam(defaultValue = "10000") double radius, Pageable pageable) {
    ...
    public ResponseEntity<PagedResponse<PostDto>> getByDestination(
            @PathVariable String countryCode, Pageable pageable) {
    ...
    public ResponseEntity<PagedResponse<PostDto>> search(@RequestParam String q, Pageable pageable) {
        return ResponseEntity.ok(PagedResponse.from(postService.search(q, pageable)));
    }

    @GetMapping("/user/{userId}")
    @Operation(summary = "Get posts by user")
    public ResponseEntity<PagedResponse<PostDto>> getByUser(@PathVariable UUID userId, Pageable pageable) {
